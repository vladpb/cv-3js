...
async function runUpgrades(options, timeout) {
    var _a;
    const [packageInfos, workspacePackages] = await (0, getAllPackages_1.default)(options);
    const packageFilepaths = packageInfos.map((packageInfo) => packageInfo.filepath);
    ...
    if (options.global) {
        ...
        analysis = await packageInfos.reduce(async (previousPromise, packageInfo) => {
