...
const findNpmConfig = (path) => {
    ...
    return (0, exports.normalizeNpmConfig)(config);
};
...
const getPeerDependencies = async (packageName, version) => {
    ...
    const peerDependencies = parseJson(result, { command: `${npmArgs.join(' ')} --json` });
...
const distTag = async (packageName, currentVersion, options = {}, npmConfig) => {
    var _a;
    const packument = (await viewOne(packageName, `dist-tags.${options.distTag}`, currentVersion, options, npmConfig)); // known type based on dist-tags.latest
